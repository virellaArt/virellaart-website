...
function routeImages(route: string): string[] {
  const productKey =
    productRoutes[route as keyof typeof productRoutes];
  ...
  const urls = allRoutes
    .flatMap((route) =>
      languageCodes.map((language) => {
        ...
        return `<url><loc>${absoluteURL(route, language)}</loc>${alternates}${xDefault}${images}</url>`;
      }),
    )
    .join("");
